...
(function (AnimationState) {
    ...
    AnimationState[AnimationState["START_WALKING"] = 2] = "START_WALKING";
    AnimationState[AnimationState["STOP_WALKING"] = 3] = "STOP_WALKING";
...
class SpriteController {
    constructor() {
        ...
        this.idleTimeout = 5000; // 5 seconds of no movement enters autonomous mode
        this.isAutonomous = false;
        this.autonomousTargetPosition = null;
        this.spriteWidth = 32; // Half the original size (64px -> 32px)
        this.spriteHeight = 32; // Half the original size (64px -> 32px)
        this.spriteElement = document.getElementById('sprite');
        this.containerElement = document.getElementById('sprite-container');
    ...
    setupEventListeners() {
        ...
        setInterval(() => {
            console.log('Debug: Mouse position =', this.lastMousePosition, '| Sprite position =', this.position, '| Target position =', this.targetPosition);
        }, 3000);
    }
    onMouseMove(e) {
    ...
    handleMouseMove(x, y) {
        ...
        const distance = Math.sqrt(Math.pow(this.targetPosition.x - this.position.x, 2) +
    ...
    setState(state) {
        ...
        switch (state) {
            case AnimationState.START_WALKING:
                ...
                this.playTransitionAnimation(this.direction === Direction.LEFT ? animations.startWalkLeft : animations.startWalkRight, AnimationState.WALKING);
                break;
            case AnimationState.WALKING:
                // Start walking animation
                this.startAnimation(this.direction === Direction.LEFT ? animations.walkLeft : animations.walkRight);
                break;
            case AnimationState.STOP_WALKING:
                // Play stop walking animation, then transition to idle
                this.playTransitionAnimation(this.direction === Direction.LEFT ? animations.stopWalkLeft : animations.stopWalkRight, AnimationState.IDLE);
                break;
            case AnimationState.IDLE:
                // Start idle animation
                this.startAnimation(this.direction === Direction.LEFT ? animations.idleLeft : animations.idleRight);
                break;
        }
    }
    ...
    playTransitionAnimation(animation, nextState) {
        // Stop current animation if it exists
        if (this.animationInterval !== null) {
            window.clearInterval(this.animationInterval);
        }
